...
async def filter(client, message):
    ...
    if 2 < len(message.text) < 100:    
        btn = []
        search = message.text
        zaute_km = f"**[🗂️](https://telegra.ph/file/4c24e39eae875f5db8048.jpg) Title:** {search}\n**⭐ Audio Rating:** {random.choice(RATING)}\n**🎭 Genre:** {random.choice(GENRES)}\n**📤 Uploaded by {message.chat.title}**"
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                await query.answer("vovo slow down buddy I Like Your Smartness, But Don't Be Oversmart 😒",show_alert=True)
                return
            ident, file_id = query.data.split("#")
            filedetails = await get_file_details(file_id)
            for files in filedetails:
